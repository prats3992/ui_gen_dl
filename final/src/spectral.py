...
class L2normalizedRandomNormal(tf.initializers.TruncatedNormal):
    ...
    def __init__(self, seed=None):
        super(L2normalizedRandomNormal, self).__init__(
            mean=0.0, stddev=1.0, seed=seed)

    def __call__(self, shape, dtype=tf.dtypes.float32):
        initial = super(L2normalizedRandomNormal,
                        self).__call__(shape, dtype=dtype)
...
class SpectralConv2D(tf.keras.layers.Conv2D):
    ...
    def __init__(self, filters, kernel_size, power_iterations=1, **kwargs):
        super(SpectralConv2D, self).__init__(filters, kernel_size,
        ...
        if not self._made_params():
            ...
            self.v = self.add_weight(name=self.name + '_v',
                                     shape=[self.kernel_size[0] *
                                            self.kernel_size[1] * input_dim, 1],
                                     initializer=L2normalizedRandomNormal(),
                                     trainable=False)
    ...
    def call(self, inputs):
        ...
        strides = [1, self.strides[0], self.strides[1], 1] if self.data_format == 'channels_last' else [
            1, 1, self.strides[0], self.strides[1]]
        outputs = tf.nn.conv2d(inputs, new_kernel, strides=strides, padding=self.padding.upper(
...
class SpectralConv2DTranspose(tf.keras.layers.Conv2DTranspose):
    def __init__(self, filters, kernel_size, power_iterations=1, **kwargs):
        super(SpectralConv2DTranspose, self).__init__(filters=filters,
    ...
    def build(self, input_shape):
        super(SpectralConv2DTranspose, self).build(input_shape)
