...
class SelfAttnWrapper(tf.keras.layers.Layer):
    def __init__(self, filters, **kwargs):
        super(SelfAttnWrapper, self).__init__(**kwargs)
        self.attn_model = SelfAttnModel(filters)

    def call(self, inputs):
        output, attention = self.attn_model(inputs)
        return output, attention  # Maintain tuple outputs for explicit handling
...
def create_generator(image_size=64, z_dim=100, filters=64, kernel_size=4, num_of_categories=5):
    with tf.device('/GPU:0'):
        ...
        for i in range(3):
            ...
            x = SpectralConv2DTranspose(
                filters=curr_filters, kernel_size=kernel_size, strides=strides, padding="same")(x)
            x = tf.keras.layers.BatchNormalization()(x)
            x = tf.keras.layers.ReLU()(x)

        x, attn1 = SelfAttnWrapper(curr_filters)(x)

        for i in range(repeat_num - 4):
            curr_filters = curr_filters // 2
            x = SpectralConv2DTranspose(
                filters=curr_filters, kernel_size=kernel_size, strides=2, padding="same")(x)
            x = tf.keras.layers.BatchNormalization()(x)
            x = tf.keras.layers.ReLU()(x)

        x, attn2 = SelfAttnWrapper(curr_filters)(x)

        x = SpectralConv2DTranspose(
            filters=3, kernel_size=kernel_size, strides=2, padding="same")(x)
