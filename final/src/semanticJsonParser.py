...
class SemanticJsonParser:
            ...
            for name in os.listdir(src):
                if ".json" in name:
                    with open(os.path.join(src, name), "r") as f:
                        ...
                        k = name[:-5]
                        SemanticJsonParser.add_to_dict(
                            k, img_json_dict, "json", data)
                elif ".jpg" in name:
                    img = cv2.imread(os.path.join(src, name), cv2.IMREAD_COLOR)
                    k = name[:-4]
                    SemanticJsonParser.add_to_dict(
                        k, img_json_dict, "img", img)
        ...
        for box in meta:
            ...
            if color and SemanticJsonParser.valid_area(start, end):
                ...
                if border:
                    ...
                    img = cv2.rectangle(
                        img, start, end, col, thickness=thickness)
        return img
    ...
    def parse_data(img_json_dict, label_hierarchy_map, label_color_map, dst):
        for key, value in img_json_dict.items():
            try:
                ...
                img = SemanticJsonParser.mark_image(
                    img_z, meta, None, label_color_map, border=True, skip_box=True)
                img = SemanticJsonParser.remove_top_bottom(img)
                SemanticJsonParser.save_img(
                    img, path=os.path.join(dst, key + ".jpg"))
            except Exception as e:
                print(key, e)
    ...
    def convert_bb_to_image(
        ...
        for i in range(len(predictions_bb)):
            ...
            img = SemanticJsonParser.mark_image(
                img, meta, label_hierarchy_map, label_color_map, gan_output=True)
            if combined:
                all_images[i] = img
            else:
                img = SemanticJsonParser.remove_top_bottom(img)
                SemanticJsonParser.save_img(
                    img, path=os.path.join(
                        dst, "predicted_" + str(ctr) + "_" + str(i) + ".jpg")
                )
        if combined:
            all_images = all_images.reshape((-1, img_w, 3))
            SemanticJsonParser.save_img(all_images, path=os.path.join(
                dst, "predicted_epoch" + str(ctr) + ".jpg"))
...
if __name__ == "__main__":
    ...
    for dp in os.listdir(src):
        ...
        if not os.path.exists(dst1):
            os.makedirs(dst1)
        SemanticJsonParser.parse_data(
            img_json_dict, label_hierarchy_map, label_color_map, dst1)
