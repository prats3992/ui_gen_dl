...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = self.to_qkv(x).view(batch, 3, self.num_heads, channels // self.num_heads, height, width)
        q, k, v = qkv.unbind(1)
        attn = (q * self.scale) @ k.permute(0, 1, 2, 4, 3)
...
for ui_type in dataset.class_to_idx.keys():
    ...
    for i, img in enumerate(generated_images):
        save_path = f"new_diff/generated_{ui_type}_{i + 1}.png"
        plt.figure()
        plt.axis('off')
