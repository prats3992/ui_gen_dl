        ...
        for line in data:
            ...
            if len(img_name) > 0:
                img_path = os.path.join(
                    android_elements_base_path, img_name + ".jpg")
                if not os.path.exists(img_path):
                    img_path = os.path.join(
                        android_elements_base_path, img_name + ".png")
                if not os.path.exists(img_path):
                    print(f"Image not found for label {label}: {img_name}")
                    img = None
                else:
            ...
            resize = int(s[3])
            android_label_map[label] = {
                "img": img, "text": text, "resize": resize, "label": label}
...
def get_elements(path, real):
    ...
    try:
        ...
        with open(path, "r") as f:
            data = json.load(f)
            if real:
                return read_json(data, label_hierarchy_map)
            else:
                shapes = data.get("shapes", [])  # Safely get shapes
                # Safely handle flags; if it doesn't exist, use an empty dictionary
    ...
    for label, bb in elements:
        ...
        if x1 >= 0 and y1 >= 0 and x2 < img_w and y2 < img_h and w > 0 and h > 0:
            ...
            if label in ["navigation_dots", "sort, heart_icon", "sort", "rating", "filter"]:
                ...
                base_shade = 224
            # print(label)
            label_image, fw = element_resize(
                label_image, w, h, label_resize, base_shade)
            if label == "image" or label == "icon":
                cv2.line(label_image, (0, 0), (w - 1, h - 1),
                         (79, 79, 79), thickness=1)
                cv2.line(label_image, (0, h - 1), (w - 1, 0),
                         (79, 79, 79), thickness=1)
            if label_resize == 4:
                fw = 0
            if label_text is not None:
                ...
                if label in element_counted.keys():
                    ...
                    if len(label_text) > c:
                        text = label_text[c]
                label_image, text_ignored = add_text_pil(
                    label_image, text, 0, label_resize, fw)
                if text_ignored and label_resize == 3:
                    label_image, fw = element_resize(
                        base_label_image, w, h, flag=2, base_shade=189)
            try:
...
def find_font_scale_pil(fontScale, h, label_text, w, reduce_text):
    ...
    while fontScale > 1 and (text_height > h * 0.90 or text_width > w * 0.85):
        fontScale -= 1
    ...
    return font, (text_width, text_height), label_text, fontScale
...
def add_text_pil(label_image, label_text, align, label_resize, fw):
    if label_text is not None and len(label_text) > 0:
        ...
        if label_resize == 3 and fw > 0 and w > fw:
            ...
            accesible_w = w
        font, textsize, label_text, fontScale = find_font_scale_pil(
            fontScale, h, label_text, accesible_w, reduce_text)
        ...
        if align == 0:  # center
            ...
            draw.text((textX, textY - 1), label_text,
                      font=font, fill=(51, 51, 51, 0))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--img_h", help="image height", type=int, default=576)

    parser.add_argument(
        "--json_file_location", "-l", help="path to category/json_files", default="./results"
    )

    parser.add_argument("--human_generated", "-r",
                        help="use if the files are generated by humans", action="store_true")
    ...
    parser.add_argument(
        ...
        default="./resources/android_elements",
    )
    parser.add_argument("--destination_folder", "-d",
                        help="path to destination", default="./final")
    parser.add_argument("--font_path", "-f",
                        help="complete path to font ttf file", default="/mnt/c/Windows/Fonts/Arial.ttf")
    parser.add_argument(
    ...
    label_hierarchy_map = UILabelFileManager.get_hierarchy_label_map(
        label_hierarchy_file, level)
    android_label_map = load_all_ui_images()

    # Walk through all subdirectories and files
    for root, _, files in os.walk(json_location):
        for file in files:
            if file.endswith(".json"):
                print(file)
                json_path = os.path.join(root, file)
                try:
                    # Extract elements and label dynamically
                    elements = get_elements(json_path, real)
                    if not elements:
                        continue

                    # Create images based on the actual label from JSON
                    dst_file_path = os.path.join(
                        dst_folder, f"{os.path.splitext(file)[0]}_{real}.jpg"
                    )
                    create_img(elements, dst_file_path, None,
                               real)  # `None` for category
                    print(f"Processed: {file} -> {dst_file_path}")
                except Exception as e:
